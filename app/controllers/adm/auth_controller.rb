...
module Adm
  class AuthController < ApplicationController
    ...
    def login
      ...
      if @email != ""
        ...
        if @password != ""
          ...
          @jumlah = AuthAdmin.where(email: @email).count
          if @jumlah == 0
            app_fail_render('Silahkan mendaftar terlebih dahulu.')
          else
            @orang = AuthAdmin.where(email: @email).first
            if @orang && @orang.authenticate(@password)
              if @orang.is_email_verify == true 
                if @orang.is_active == true
                  ...
                  app_fail_render('Akun Anda tidak aktif')
                end
              else  
    ...
    def register
      ...
      if @password == @password_confirm
        ...
        if(@email =~ valid_email_regex) == 0
          ...
          if AuthAdmin.where(email: @email).count == 0
            ...
            @orang = AuthAdmin.find_by(email: @email)
            if @orang.is_email_verify == true
    ...
    def register_vercode
      ...
      @orangs = AuthAdmin.where(email: @email)
    ...
    def forgot_password
      ...
      @count = AuthAdmin.where(email: @email).count
    ...
    def change_password
      ...
      if @password == @password_confirm
        ...
        @jumlah = AuthAdmin.where(email: @email, pass_reset_token: @token).count

        if @jumlah == 0
          app_fail_render('Kombinasi email dan token tidak benar')
        else
          @orang = AuthAdmin.where(email: @email, pass_reset_token: @token).first
